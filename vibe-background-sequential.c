...
int32_t libvibeModel_Sequential_AllocInit_8u_C1R(
 ...
 //and define int i; int index; solve for loop initial declaration only fo c99
  unsigned int i;
  int index;
  for (i = 0; i < NUMBER_OF_HISTORY_IMAGES; ++i) {
  ...
  for (index = width * height - 1; index >= 0; --index) {
    uint8_t value = image_data[index];
        //define x for for loop
       unsigned int x;
    for (x = 0; x < model->numberOfSamples - NUMBER_OF_HISTORY_IMAGES; ++x) {
      int value_plus_noise = value + rand() % 20 - 10;  //value + (-10~10) random number
  ...
  unsigned int size = (width > height) ? 2 * width + 1 : 2 * height + 1;
...
int32_t libvibeModel_Sequential_Segmentation_8u_C1R(
  ...
  for (index = width * height - 1; index >= 0; --index) {
    if (segmentation_map[index] > 0) {
      ...
      for (i = numberOfTests; i > 0; --i, ++indexHistoryBuffer) {
...
int32_t libvibeModel_Sequential_AllocInit_8u_C3R(
  ...
  int index;
  for (i = 0; i < NUMBER_OF_HISTORY_IMAGES; ++i) {
    for (index = (3 * width) * height - 1; index >= 0; --index)
...
int32_t libvibeModel_Sequential_Segmentation_8u_C3R(
  ...
  for (index = width * height - 1; index >= 0; --index) {
    if (segmentation_map[index] > 0) {
      ...
      for (i = numberOfTests; i > 0; --i, indexHistoryBuffer += 3) {
        if (
          distance_is_close_8u_C3R( 
            image_data[(3 * index)], image_data[(3 * index) + 1], image_data[(3 * index) + 2], 
