...
//    cv::Point ofs;
 //   cv::Size  size;
  ...
  //  hogImage.locateROI(size,ofs);
  //  std::cout<<"initial roi ofset x="<<ofs.x<<"y="<<ofs.y<<std::endl;
    ...
    for (int i = 0; i < image.rows / cellSize.height; i++) {
        for (int j = 0; j < image.cols / cellSize.width; j++) {
            // draw wach cell
            subImage=hogImage(cv::Rect(j*cellSize.width, i*cellSize.height, cellSize.width, cellSize.height));

       //     subimage.locateROI(size,ofs);
       //     std::cout<<"roi ofset x="<<ofs.x<<"y="<<ofs.y<<std::endl;

            drawHOG(itDesc, nBins, subImage, scale);
